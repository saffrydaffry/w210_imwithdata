...
def twitter(df, conn):
    ...
    actions['city_state'] = actions.apply(lambda row: city_state_process(row.tweet_cities, row.tweet_states),
                                          axis=1
                                          )

    ### SEND DATA TO SQL
    prepped_to_sql = pd.DataFrame(
        {'id': actions['id'].tolist(),
         'title': [tweet.capitalize().encode('utf-8') if tweet.isupper() else tweet.encode('utf-8') for tweet in actions['tweet'].tolist()],
         ...
         'city':actions['tweet_cities'].tolist(),
...
def meetup(meetup_df, conn):
    ...
    for index, row in tqdm(meetup_df.iterrows()):
        ...
        try:
            ...
            temp_row['event_legislator'] = associated_legislators
            ...
            temp_row['event_description'] = str(row['_source.description']).encode('utf-8')
            ...
            temp_row['event_full_address'] = (str(temp_row['event_location_name']) + ', ' +
                                              str(temp_row['event_address']) + ', ' +
                                              str(temp_row['event_city']) + ', ' +
                                              str(temp_row['event_state']) + ' ' +
                                              str(temp_row['event_zip']))
